<template>
  <div class="dp-invite-friend-content">
    <p class="dp-invite-friend-content__hint">
      将向好友发送 60 秒内有效的进房邀请；对方在大厅邮箱接受后可免密码以观众身份进入本房间。
    </p>
    <p
        v-if="!friendsLoading && !selectableFriends.length"
        class="dp-invite-friend-content__empty"
    >
      暂无好友可邀请（需已成功添加好友）。
    </p>
    <ul
        v-else-if="selectableFriends.length"
        class="dp-invite-friend-content__list"
    >
      <li
          v-for="f in selectableFriends"
          :key="f.userId"
          :class="['dp-invite-friend-content__row', friendInvitePresenceClass(f)]"
      >
        <button
            type="button"
            class="dp-invite-friend-content__left"
        ...
        >
          <dp-user-avatar
            ...
            size="sm"
          />
          <div class="dp-invite-friend-content__meta">
            <span
                class="dp-invite-friend-content__name"
            ...
            <span
                ...
                class="dp-invite-friend-content__presence"
        ...
        <el-button
            type="primary"
            size="small"
            class="dp-invite-friend-content__invite-btn"
...
import { dpResultSuccess, dpResultMessage, dpAxiosErrorMessage } from '@shared/utils/dpApiResult'
  ...
  methods: {
    ...
    async opened() {
      ...
      try {
        ...
        if (r && r.ok) {
          dpInviteFriendsDevLog('fetch ok', { friendsCount: n })
...
<style scoped>
.dp-invite-friend-content {
  padding: 4px 2px 8px;
  max-width: 100%;
  margin: 0 auto;
}

.dp-invite-friend-content__hint {
  font-size: 13px;
...
.dp-invite-friend-content__empty {
  font-size: 13px;
  color: var(--dp-text-muted);
  line-height: 1.5;
  margin: 8px 0 0;
...
.dp-invite-friend-content__list {
  list-style: none;
  margin: 0;
  padding: 0;
...
.dp-invite-friend-content__row:last-child {
  border-bottom: none;
}

.dp-invite-friend-content__left {
  flex: 1 1 auto;
  min-width: 0;
  display: flex;
  flex-direction: row;
...
.dp-invite-friend-content__left:hover,
.dp-invite-friend-content__left:focus-visible {
...
.dp-invite-friend-content__meta {
  flex: 1 1 auto;
  min-width: 0;
  display: flex;
...
.dp-invite-friend-content__name {
  width: 100%;
  font-size: 14px;
...
.dp-invite-friend-content__presence {
  font-size: 12px;
  font-weight: 600;
  line-height: 1.35;
...
.dp-invite-friend-content__row.dp-friend-row--presence-idle .dp-invite-friend-content__presence {
  color: #529b2e;
}

.dp-invite-friend-content__row.dp-friend-row--presence-ingame .dp-invite-friend-content__presence {
  color: #dd6161;
}

.dp-invite-friend-content__row.dp-friend-row--presence-offline .dp-invite-friend-content__presence {
  color: #9a9ea4;
}

.dp-invite-friend-content__invite-btn {
  flex-shrink: 0;
  min-width: 64px;
  padding-left: 12px;
  padding-right: 12px;
