 ...
 * 上游枚举名：IDLE / IN_GAME / OFFLINE（见 DpFriendPresenceState）。
 *
 * @param {Record<string, unknown> | null | undefined} f
 * @returns {'idle' | 'in_game' | 'offline' | 'unknown'}
 */
...
export function dpFriendPresenceRowClass(f) {
  ...
  if (b === 'offline') return 'dp-friend-row--presence-offline'
  return ''
}
