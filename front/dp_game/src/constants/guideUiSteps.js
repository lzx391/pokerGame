...
/** retro8bit：标题对齐顶栏/底栏英文 UI，正文仍为中文说明 */
var RETRO_GUIDE_PATCH_BY_REF = Object.freeze({
  guideTopSettings: {
    title: 'SETTINGS',
    body: '换界面主题、开关节能模式（少动画更省电）。真对局里随时能改，只影响你自己屏幕上的样子。'
  },
  guideTopStage: {
    title: 'PHASE',
    body:
      '当前牌局进行到哪一段：翻前圈 → 翻后圈 → 半决赛 → 决赛圈 → 结算阶段。公共牌一张张翻开时，顶栏 PHASE 会显示 PREFLOP、FLOP 等英文阶段名。'
  },
  guideTopPot: {
    title: 'POT',
    body: '本局大家投进桌心的筹码总量（顶栏 POT 数字）。赢家（或最后没盖牌的人）从这里分走奖励。'
  },
  guideTopHeroEco: {
    title: 'STACK',
    body:
      '顶栏 STACK 是你还能用的筹码；BET / TO CALL 行显示本轮已投或还需补多少；INVESTED 是本局已消耗。'
  },
  guideTopAlign: {
    title: 'TO CALL',
    body: '桌上当前最高投入（顶栏 TO CALL）。你要跟注或加注，总投入得对齐这个数（或更高），否则不能争 POT。'
  },
  guideTopFullscreen: {
    title: 'FULLSCREEN',
    body: '铺满屏幕，牌桌和底栏按钮更大。进教程会和真对局一样自动尝试全屏；再点一次可退出。'
  ...
  guideTopOwnerHub: {
    ...
    title: 'OWNER',
    body: '只有房主可见：顶栏 OWNER 按钮可打开房主操作（加演示 NPC、踢人、移交房主、看穿底牌等）。教程里按钮在，但不会真改房间。'
  },
  guideTopInvite: {
    title: 'INVITE',
    body: '给互为好友的玩家发进房邀请。对方同意后可直接进这间房（观战或上桌视规则而定）。'
  ...
  guideTableWrap: {
    title: 'TABLE',
    body:
      '椭圆桌：中间是公共牌与 POT；周围是各座位的手牌区。发、SC、BC 小标是庄家与盲注位；轮到你行动时座位会高亮。'
  },
  guideMobileChatCluster: {
    title: 'CHAT',
    body: '左下角和真对局一样：点「聊天」可展开记录，下面输入想说的话。不会真发到服务器。'
  },
  guideChatToggle: {
  ...
  guideMobileLeaveSeat: {
    title: 'LEAVE SEAT',
    body: '本手打完前离开座位去观众席（本手算 FOLD）。想再玩可报名「下一局加入对局」。'
  },
  guideMobileActionBtn: {
    title: 'ACTION',
    body: '轮到你时，先点底栏「行动（倒计时）」打开抽屉。教程马上会帮你模拟点一下，和真对局一样。'
  },
  guideActionSheet: {
    title: 'ACTION PANEL',
    body: '打开后整块抽屉里：上面快捷加注和滑条，中间 CALL/CHECK 与 RAISE，下面 ALL-IN 和 FOLD。下面几步会逐个特写。'
  },
  guidePotPresets: {
    title: 'QUICK BET',
    body: '按 POT 比例（⅓池、½池…）或「最小加注」快速填好加注额。'
  },
  guideRaiseSlider: {
    title: 'BET AMOUNT',
    body: '拖动滑条微调这一笔投多少；右边数字会跟着变。'
  ...
  guideCall: {
    title: 'CALL / CHECK',
    body: '有人加注时跟上 TO CALL 才能继续；没人加注时显示「观望」= 过牌（CHECK）。'
  },
  guideRaise: {
    title: 'RAISE',
    body: '在跟齐之外再多投，逼别人跟更多筹码进池。'
  },
  guideFold: {
    title: 'FOLD',
    body: '放弃本手，不再争 POT。已投进去的不退。'
  },
  guideAllin: {
    title: 'ALL-IN',
    body: '把手牌区显示的 STACK 一次全压。'
  },
  guideActionTimer: {
    title: 'TIMER',
    body: '圆环倒数约 30 秒；超时自动 FOLD，避免拖局。'
  },
  guideNotMyTurn: {
    title: 'WAIT TURN',
    body: '底栏没有「行动」按钮或抽屉打不开，只能看别人打。顶栏仍可看 POT 与 TO CALL。'
  }
})

var RETRO_GUIDE_COMPLETE = Object.freeze({
  title: 'UI TOUR COMPLETE',
  body: '可以去大厅「创建房间」或「快速匹配」实战。牌型与完整流程点顶栏 RULES。',
  note: '本页为单机演示，所有按钮不会连服务器。'
})

function applyRetroGuidePatch(step) {
  var patch = RETRO_GUIDE_PATCH_BY_REF[step.ref]
  if (!patch) return Object.assign({}, step)
  return Object.assign({}, step, patch)
}

/** @param {string} [theme] store dpGame.gameUiTheme */
export function getGuideSteps(theme) {
  if (theme !== 'retro8bit') return GUIDE_UI_STEPS.slice()
  return GUIDE_UI_STEPS.map(applyRetroGuidePatch)
}

/** @param {string} [theme] */
export function getGuideCompleteStep(theme) {
  if (theme !== 'retro8bit') return GUIDE_UI_COMPLETE_STEP
  return Object.assign({}, GUIDE_UI_COMPLETE_STEP, RETRO_GUIDE_COMPLETE)
}
