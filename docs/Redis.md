...
取->音乐服务实现类调用音乐缓存服务接口，把key输入，取出数据，如果没有数据再从Mapper接口即数据库里查出来并更新Redis  
  ...
  ---
  # 房间列表模块+redis
  思路是这样的，首先分两类方式，一种是不带条件默认搜索，键设计是 页码：页大小->房间VO的json  
  一种是带条件搜索，这种键设计是把查询条件和分页信息用哈希算法加工，作为K，然后里面存对应的房间VO的json，下次访问的时候，前端传入查询条件和分页信息，后端先哈希加工，然后去redis找，没有的话再从数据库查并更新redis