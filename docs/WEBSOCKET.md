...
| **房间聊天（短时气泡）** | 与房间快照 **共用** 同一条 `ws` 连接。客户端发 `{"_ws":"chatSend","nickname":"…","text":"…"}`（`text` 最长 200 字、约 1.2 秒一条）；服务端校验昵称属于当前房间 **玩家或观众** 后广播 `{"_ws":"chat",…,"ttlMs":15000}`，**不写数据库**。对局页按昵称 **只保留一条展示**（新发顶旧），**桌上玩家**文案显示在该座位卡片上方，**观众**显示在操作区上方条带；输入框与 **行动面板** 同列且在按钮 **下方**。 |
| **房间曲库 BGM（音乐盒）** | 客户端发 `{"_ws":"roomMusicSync","nickname":"…","action":"play"|"pause"|"stop","trackId":…,"webPath":"/music/…","displayName":"…"}`（`play` 时须带合法 `trackId` 与 `webPath`；约 400ms 内同连接节流）。服务端广播 `{"_ws":"roomMusic","action":"…","byNickname":"…","serverTime":…,"trackId":…,"webPath":…,"displayName":…}`，并在内存中记下该房间最后一帧；**新订阅者**在收到首包房间 JSON 后再收到该帧（若存在）。**不参与**与 `getNowRoom` 快照相同的去重逻辑。 |
