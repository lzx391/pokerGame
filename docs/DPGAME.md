# DP 对局后端维护笔记（接口、房间与 NPC）

> **编者约定**：本文为作者对德扑对局后端代码的理解笔记，便于后续维护；由 AI 修改前需征得开发者许可。

**本篇说明**：面向需要维护后端对局逻辑的读者；内容与 `DpRoomServiceImpl`、房间 REST、NPC 等行为对应。**编程备忘**（原文）：新开/结束要记得重置状态；重复逻辑宜封装；在 `action` 相关路径上要对「本轮最后行动的玩家」做好判定以推进局面。
