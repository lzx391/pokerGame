...
public interface DpQuickMatchPairingHost {
    ...
    DpRoomBO getRoom(String roomId);

    /**
     * Redis SSOT room mutation under distributed lock (replaces {@code synchronized(DpRoomBO)}).
     */
    <T> T runExclusiveRoom(String roomId, Function<DpRoomBO, T> action);

    long nowMillis();

    /**
     * 在已持全局配对锁下：修剪超时 / 已在房内者，并返回因<strong>超时</strong>被移出者的昵称（供推送）。
     */
    ...
    /**
     * 在已 {@code runExclusiveRoom} 内：完成该玩家进房与准备（对齐历史
