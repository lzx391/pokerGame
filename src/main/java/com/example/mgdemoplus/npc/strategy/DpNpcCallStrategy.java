...
public final class DpNpcCallStrategy {
    ...
    public static BotAction decide(DpNpcRuleDecisionParams p) {
        // System.out.println("DpNpcCallStrategy");
        ...
        if (!"preflop".equals(stage)) {
            ...
            DpNpcEngine.updateHandPlanForLaterStreetIfNeeded(
                    p.room, p.bot, BotType.CALL, p.strength, p.boardDanger, fishCallCtx);
        ...
        if (p.callAmount > 0 && !"preflop".equals(stage)) {
            ...
            if (p.strength == SimpleStrength.WEAK && p.callRatio > 0 && p.callRatio < 0.38) {
                foldBase *= 0.52;
            }
            if (fishCallPlan == HandPlanType.GIVE_UP) {
                foldBase = Math.min(1.0, foldBase + 0.06);
            }
            if (fishCallCtx != null) {
                foldBase = Math.min(
                        1.0,
            ...
            if (fishCallCtx != null && fishCallCtx.potOdds > 0
                    && fishCallCtx.potOdds < 0.34 && p.callRatio < 0.55
                    && (p.strength == SimpleStrength.MEDIUM || p.strength == SimpleStrength.STRONG
                            || p.strength == SimpleStrength.MONSTER)) {
                foldBase *= 0.68;
            }
        ...
        double callOrCheckProb = 0.80 - p.mood * 0.1;
        if (!"preflop".equals(stage)) {
