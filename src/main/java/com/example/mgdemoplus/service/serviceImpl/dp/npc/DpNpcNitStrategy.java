...
/**
 * 紧弱（NIT）：翻后规则独立维护（与 TAG 分文件，不复用 boolean 分支）。
 */
public final class DpNpcNitStrategy {
    ...
    public static BotAction decide(DpNpcRuleDecisionParams p) {
        // System.out.println("DpNpcNitStrategy");
        ...
        DpNpcEngine.initHandPlanIfNeededForPostflop(
                ...
                ctx,
                p.random);
        DpNpcEngine.updateHandPlanForLaterStreetIfNeeded(p.room, p.bot, BotType.NIT, st, bd, ctx);
