...
- **机器人决策随机**：规则 NPC 决策、思考延时、桌边话术均使用无种子纯随机（`ThreadLocalRandom`），与 `currentHandSeed` 无关；房间的 `currentHandSeed` 仍用于前端动画 key、牌谱等，**不是发牌种子**，洗牌在 `newDeck()` 里单独 `Collections.shuffle`。
